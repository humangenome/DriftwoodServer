...
| **`TargetFrameRate` (50-60)** | 64.9% -> **11.8%** with a player connected | **−82% on a loaded server** | The engine ignores `Application.targetFrameRate` in batch mode, so the host pads the frame itself — check `frameLimiterActive` and `actualFrameRate`, never the engine's own value. **Do not use 30:** it is below the 50 Hz netcode tick and batches sends. Set it above the tick rate, or measure the smoothness cost first. |
